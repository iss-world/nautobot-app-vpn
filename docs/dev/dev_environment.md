...
- `nautobot_ver`: the version of Nautobot to use as a base for any built docker containers (default: 3.0.1)
- `project_name`: the default docker compose project name (default: `nautobot_app_vpn`)
...
Using **Invoke** these configuration options can be overridden using [several methods](https://docs.pyinvoke.org/en/stable/concepts/configuration.html). Perhaps the simplest is setting an environment variable `INVOKE_NAUTOBOT_APP_VPN_VARIABLE_NAME` where `VARIABLE_NAME` is the variable you are trying to override. The only exception is `compose_files`, because it is a list it must be overridden in a YAML file. There is an example `invoke.yml` (`invoke.example.yml`) in this directory which can be used as a starting point.
...
#14 naming to docker.io/nautobot_app_vpn/nautobot:3.0.1-py3.11 done
```
...
ee90fbfabd77   nautobot_app_vpn/nautobot:3.0.1-py3.11  "nautobot-server rqw…"   16 seconds ago   Up 13 seconds                                               nautobot_app_vpn_worker_1
b8adb781d013   nautobot_app_vpn/nautobot:3.0.1-py3.11  "/docker-entrypoint.…"   20 seconds ago   Up 15 seconds   0.0.0.0:8080->8080/tcp, :::8080->8080/tcp   nautobot_app_vpn_nautobot_1
d64ebd60675d   nautobot_app_vpn/nautobot:3.0.1-py3.11  "mkdocs serve -v -a …"   25 seconds ago   Up 18 seconds   0.0.0.0:8001->8080/tcp, :::8001->8080/tcp   nautobot_app_vpn_docs_1
...
So for example, our app is named `nautobot_app_vpn`, the command would most likely be `docker logs nautobot_app_vpn_nautobot_1 -f`. You can find the name of all running containers via `docker ps`.
...
Or set the `INVOKE_NAUTOBOT_APP_VPN_PYTHON_VER` variable.
...
Or set the `INVOKE_NAUTOBOT_APP_VPN_NAUTOBOT_VER` variable.
...
In the package source, there is the `nautobot_app_vpn/app-config-schema.json` file, conforming to the [JSON Schema](https://json-schema.org/) format. This file is used to validate the configuration of the app in CI pipelines.
