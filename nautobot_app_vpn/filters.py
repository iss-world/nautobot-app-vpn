...
from nautobot_app_vpn.models import (
    IKECrypto, IPSecCrypto, IKEGateway, IPSECTunnel, IPSecProxyID,
...
from nautobot_app_vpn.models.constants import ( # Shortened for brevity
    EncryptionAlgorithms, AuthenticationAlgorithms, DiffieHellmanGroups, IPSECProtocols,
    IKEAuthenticationTypes, LifetimeUnits, IKEVersions, IKEExchangeModes, IdentificationTypes, IPAddressTypes
)
...
class IKECryptoFilterSet(BaseFilterSet):
    ...
    class Meta: model = IKECrypto; fields = ["id", "q", "name", "description", "dh_group", "encryption", "authentication", "lifetime", "lifetime_unit", "status"]
...
class IPSecCryptoFilterSet(BaseFilterSet):
    ...
    lifetime = django_filters.RangeFilter()
    lifetime_unit = django_filters.ChoiceFilter(choices=LifetimeUnits.choices)
    class Meta: model = IPSecCrypto; fields = ["id", "q", "name", "description", "encryption", "authentication", "dh_group", "protocol", "lifetime", "lifetime_unit", "status"]


class IKEGatewayFilterSet(BaseFilterSet):
    local_devices = ModelMultipleChoiceFilter(field_name="local_devices",queryset=Device.objects.all(), label="Local Devices")
    peer_devices = ModelMultipleChoiceFilter(queryset=Device.objects.all(), label="Peer Devices", required=False) # Allow filtering with no peer
    local_locations = ModelMultipleChoiceFilter(queryset=Location.objects.all(), label="Local Locations", required=False)
    ...
    authentication_type = django_filters.MultipleChoiceFilter(choices=IKEAuthenticationTypes.choices, label="Authentication Type")
    ...
    local_platform = ModelMultipleChoiceFilter( field_name="local_platform", queryset=Platform.objects.all(), label="Local Platform(s)")
    peer_platform = ModelMultipleChoiceFilter(field_name="peer_platform", queryset=Platform.objects.all(), label="Peer Platform(s)")
    ...
    class Meta: model = IKEGateway; fields = ["id","q","name","description","local_devices","peer_devices","local_locations","peer_locations","peer_location_manual","bind_interface","local_ip","peer_ip","authentication_type","ike_crypto_profile","status","ike_version","exchange_mode","local_ip_type","peer_ip_type","local_id_type","peer_id_type","peer_device_manual","enable_passive_mode","enable_nat_traversal","enable_dpd","local_platform","peer_platform","limit", "offset", "depth", "exclude_m2m"]
...
class IPSECTunnelFilterSet(BaseFilterSet): # Keep BaseFilterSet for Status
    ...
    class Meta:
        ...
        fields = [
            "id","q","name","description","devices","ike_gateway",
            "ipsec_crypto_profile","tunnel_interface",
            "role", # Added role
