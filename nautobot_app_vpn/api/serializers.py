...
from nautobot_app_vpn.models import (
    ...
    IPSECTunnel,
    IPSecProxyID,
    TunnelMonitorProfile, TunnelMonitorActionChoices,
    VPNDashboard,
    TunnelRoleChoices,
)
from rest_framework import serializers
from nautobot_app_vpn.models.constants import IPAddressTypes, IdentificationTypes, IKEVersions, IKEExchangeModes, IKEAuthenticationTypes

# --- Nested Serializers ---
# (No changes needed in Nested Serializers)
class NestedDeviceSerializer(BaseModelSerializer):
    url = serializers.HyperlinkedIdentityField(view_name='dcim-api:device-detail')
    class Meta: model = Device; fields = ["id", "url", "display", "name"]

class NestedLocationSerializer(BaseModelSerializer):
    url = serializers.HyperlinkedIdentityField(view_name='dcim-api:location-detail')
    class Meta: model = Location; fields = ["id", "url", "display", "name"]
...
class NestedStatusSerializer(BaseModelSerializer):
    ...
    class Meta: model = Status; fields = ["id", "url", "display", "name", "color"]
...
class NestedIKECryptoSerializer(BaseModelSerializer):
    ...
    class Meta: model = IKECrypto; fields = ["id", "url", "display", "name"]
...
class NestedIPSecCryptoSerializer(BaseModelSerializer):
     ...
     class Meta: model = IPSecCrypto; fields = ["id", "url", "display", "name"]
...
class NestedIKEGatewaySerializer(BaseModelSerializer):
     ...
     class Meta: model = IKEGateway; fields = ["id", "url", "display", "name"]
...
class NestedInterfaceSerializer(BaseModelSerializer):
     ...
     class Meta: model = Interface; fields = ["id", "url", "display", "name", "device"] # Added device
...
class NestedIPSECTunnelSerializer(BaseModelSerializer):
     ...
     class Meta: model = IPSECTunnel; fields = ["id", "url", "display", "name"]
...
class IKECryptoSerializer(BaseModelSerializer):
    ...
    status_id = serializers.PrimaryKeyRelatedField(queryset=Status.objects.all(), source='status', write_only=True, required=False, allow_null=True, label="Status")
    class Meta: model = IKECrypto; fields = ["id", "display", "url", "name", "dh_group", "encryption", "authentication", "lifetime", "lifetime_unit", "status", "status_id", "description", "created", "last_updated"]; read_only_fields = ["id", "display", "url", "status", "created", "last_updated"]

class IPSecCryptoSerializer(BaseModelSerializer):
    url = serializers.HyperlinkedIdentityField(view_name="plugins-api:nautobot_app_vpn-api:ipseccrypto-detail")
    status = NestedStatusSerializer(required=False, allow_null=True, read_only=True)
    status_id = serializers.PrimaryKeyRelatedField(queryset=Status.objects.all(), source='status', write_only=True, required=False, allow_null=True, label="Status")
    class Meta: model = IPSecCrypto; fields = ["id", "display", "url", "name", "encryption", "authentication", "dh_group", "protocol", "lifetime", "lifetime_unit", "status", "status_id", "description", "created", "last_updated"]; read_only_fields = ["id", "display", "url", "status", "created", "last_updated"]
...
class IKEGatewaySerializer(BaseModelSerializer):
    ...
    local_device_ids = serializers.PrimaryKeyRelatedField(queryset=Device.objects.all(), source='local_devices', many=True, write_only=True, required=True, label="Local Devices (IDs)")
    peer_device_ids = serializers.PrimaryKeyRelatedField(queryset=Device.objects.all(), source='peer_devices', many=True, write_only=True, required=False, label="Peer Devices (IDs)")
    local_location_ids = serializers.PrimaryKeyRelatedField(queryset=Location.objects.all(), source='local_locations', many=True, write_only=True, required=False, label="Local Locations (IDs)")
    peer_location_ids = serializers.PrimaryKeyRelatedField(queryset=Location.objects.all(), source='peer_locations', many=True, write_only=True, required=False, label="Peer Locations (IDs)")
    ike_crypto_profile_id = serializers.PrimaryKeyRelatedField(queryset=IKECrypto.objects.all(), source='ike_crypto_profile', write_only=True, required=True, allow_null=False, label="IKE Crypto Profile")
    status_id = serializers.PrimaryKeyRelatedField(queryset=Status.objects.all(), source='status', write_only=True, required=False, allow_null=True, label="Status")
    ...
    # Other Fields
    pre_shared_key = serializers.CharField(write_only=True, required=False, allow_blank=True, allow_null=True, style={'input_type': 'password'})

    class Meta:
        model = IKEGateway
        # <<< UPDATED: Added bind_interface fields
        fields = [
            "id","display","url","name","description","ike_version","exchange_mode",
            "local_ip_type","local_ip","local_devices","local_device_ids","local_locations","local_location_ids",
            "local_platform", "local_platform_id","local_id_type","local_id_value",
            "peer_ip_type","peer_ip","peer_devices","peer_device_ids","peer_device_manual",
            "peer_locations","peer_location_ids","peer_location_manual","peer_platform", "peer_platform_id", "peer_id_type","peer_id_value",
            "authentication_type","pre_shared_key","ike_crypto_profile","ike_crypto_profile_id",
            "bind_interface", "bind_interface_id", # Added here
            "enable_passive_mode","enable_nat_traversal","enable_dpd","dpd_interval","dpd_retry",
            "liveness_check_interval","status","status_id","last_sync","created","last_updated"
        ]
        # <<< UPDATED: Added bind_interface to read_only
        read_only_fields = [
            "id", "display", "url",
            "local_devices", "peer_devices", "local_locations", "peer_locations", # Keep M2M nested as read-only
            "ike_crypto_profile", "status",
            "bind_interface", "local_platform", "peer_platform",
            "created", "last_updated", "last_sync"
        ]
    ...
    def validate(self, data):
        ...
        if peer_locations and peer_location_manual: raise serializers.ValidationError("Specify Peer Locations *or* Manual Peer Location, not both.")
        ...
        if bind_iface_id and local_device_ids:
            try:
                ...
                if bind_iface_obj.device.pk not in [dev.pk for dev in local_device_ids]:
                     raise serializers.ValidationError({"bind_interface_id": "Selected Bind Interface must belong to one of the selected Local Devices."})
...
class IPSecProxyIDSerializer(BaseModelSerializer):
     ...
     tunnel_id = serializers.PrimaryKeyRelatedField(queryset=IPSECTunnel.objects.all(), source='tunnel', write_only=True, required=True, allow_null=False, label="IPSec Tunnel")
     class Meta: model = IPSecProxyID; fields = ["id", "url", "display", "tunnel", "tunnel_id", "local_subnet", "remote_subnet", "protocol", "local_port", "remote_port"]; read_only_fields = ["id", "url", "display", "tunnel"]
...
class VPNDashboardSerializer(BaseModelSerializer):
     ...
     class Meta: model = VPNDashboard; fields = ["id", "url", "display", "name", "last_updated", "total_tunnels", "active_tunnels", "inactive_tunnels", "last_sync_status", "last_sync_time", "last_push_status", "last_push_time", "created"]; read_only_fields = ["id", "url", "display", "created", "last_updated"]
...
class IPSECTunnelSerializer(BaseModelSerializer):
    ...
    device_ids = serializers.PrimaryKeyRelatedField(
        queryset=Device.objects.all(), source='devices', many=True, write_only=True, required=True, label="Devices (IDs)"
    )
    ike_gateway_id = serializers.PrimaryKeyRelatedField(queryset=IKEGateway.objects.all(), source='ike_gateway', write_only=True, required=True, allow_null=False, label="IKE Gateway")
    ipsec_crypto_profile_id = serializers.PrimaryKeyRelatedField(queryset=IPSecCrypto.objects.all(), source='ipsec_crypto_profile', write_only=True, required=True, allow_null=False, label="IPSec Crypto Profile")
    status_id = serializers.PrimaryKeyRelatedField(queryset=Status.objects.all(), source='status', write_only=True, required=False, allow_null=True, label="Status")
    tunnel_interface_id = serializers.PrimaryKeyRelatedField(queryset=Interface.objects.all(), source='tunnel_interface', write_only=True, required=True, allow_null=False, label="Tunnel Interface")
    ...
    monitor_profile_id = serializers.PrimaryKeyRelatedField(
        queryset=TunnelMonitorProfile.objects.all(), source='monitor_profile', write_only=True, required=False, allow_null=True, label="Monitor Profile (ID)"
    )
    ...
    class Meta:
        ...
        fields = [
            "id", "display", "url", "name", "description",
            "devices", "device_ids",
            "ike_gateway", "ike_gateway_id",
            "ipsec_crypto_profile", "ipsec_crypto_profile_id",
            "tunnel_interface", "tunnel_interface_id",
            # "bind_interface", "bind_interface_id", # Removed
            "role",
            "proxy_ids",
            # Tunnel Monitor fields
            "enable_tunnel_monitor", "monitor_destination_ip",
            "monitor_profile", "monitor_profile_id",
            # Metadata
            "status", "status_id",
            "last_sync", "created", "last_updated"
        ]
        # --- UPDATED Read Only Fields ---
        read_only_fields = [
            "id", "display", "url",
            "devices",
            "ike_gateway", "ipsec_crypto_profile", "status",
    ...
    def validate(self, data):
        monitor_enabled = data.get('enable_tunnel_monitor', getattr(self.instance, 'enable_tunnel_monitor', False) if self.instance else False)
        dest_ip = data.get('monitor_destination_ip', getattr(self.instance, 'monitor_destination_ip', None) if self.instance else None)
        ...
        if monitor_enabled:
            if not dest_ip: raise serializers.ValidationError({"monitor_destination_ip": "Destination IP is required when tunnel monitoring is enabled."})
            # Check submitted ID for profile
            if profile_id_submitted and data.get('monitor_profile_id') is None:
                 raise serializers.ValidationError({"monitor_profile_id": "Monitor Profile is required when tunnel monitoring is enabled."})
            ...
            if self.instance and profile_id_submitted and data.get('monitor_profile_id') is None and monitor_enabled:
                 raise serializers.ValidationError({"monitor_profile_id": "Cannot remove Monitor Profile while tunnel monitoring is enabled."})
