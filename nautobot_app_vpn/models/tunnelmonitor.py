...
@extras_features(
    ...
    "graphql", "relationships", "webhooks",
)
class TunnelMonitorProfile(PrimaryModel):
    ...
    action = models.CharField(max_length=20, choices=TunnelMonitorActionChoices.choices, default=TunnelMonitorActionChoices.WAIT_RECOVER, help_text="Action to take when tunnel monitoring detects a failure.")
    interval = models.PositiveIntegerField(default=3, validators=[MinValueValidator(1)], help_text="Probe interval in seconds.")
    threshold = models.PositiveIntegerField(default=5, validators=[MinValueValidator(1)], help_text="Number of consecutive failed probes before triggering action.")
