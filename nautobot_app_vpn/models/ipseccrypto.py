...
class IPSecCrypto(PrimaryModel, ChangeLoggedModel):
    ...
    name = models.CharField(
        max_length=100,
        unique=True,
        help_text="Unique name for the IPSec Crypto Profile."
    )

    description = models.TextField(
        blank=True,
        help_text="Optional free-form description or usage notes."
    )

    encryption = models.CharField(
        max_length=50,
        choices=EncryptionAlgorithms.choices,
        help_text="Encryption algorithm used."
    )

    authentication = models.CharField(
        max_length=50,
        choices=AuthenticationAlgorithms.choices,
        help_text="Authentication algorithm used."
    )

    dh_group = models.CharField(
        max_length=50,
        choices=DiffieHellmanGroups.choices,
        help_text="Diffie-Hellman Group for key exchange."
    )

    protocol = models.CharField(
        max_length=5,
        choices=IPSECProtocols.choices,
        default=IPSECProtocols.ESP,
        help_text="IPSec protocol used."
    )

    lifetime = models.PositiveIntegerField(
        help_text="Lifetime duration (must be a positive number)."
    )
