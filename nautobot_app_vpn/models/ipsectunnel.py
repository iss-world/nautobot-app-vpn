...
@extras_features(
    "custom_fields", "custom_links", "custom_validators", "export_templates",
    "graphql", "relationships", "statuses", "webhooks",
)
class IPSECTunnel(PrimaryModel):
    """IPSec Tunnel configuration potentially spanning multiple devices."""

    name = models.CharField(
        max_length=100, help_text="Unique name for the IPSec Tunnel."
    )
    ...
    devices = models.ManyToManyField(
        ...
        help_text="Firewall device(s) associated with this IPSec Tunnel (for HA)."
    )
    # --- END MODIFICATION ---

    ike_gateway = models.ForeignKey(
        IKEGateway, on_delete=models.PROTECT, related_name="ipsec_tunnels",
        help_text="IKE Gateway associated with this IPSec Tunnel."
    )
    ipsec_crypto_profile = models.ForeignKey(
        IPSecCrypto, on_delete=models.PROTECT, related_name="ipsec_tunnels",
        help_text="IPSec Crypto Profile applied to this tunnel."
    )
    tunnel_interface = models.ForeignKey(
        "dcim.Interface", on_delete=models.PROTECT, related_name="ipsec_tunnel_interfaces",
    ...
    enable_tunnel_monitor = models.BooleanField(
        default=False, help_text="Enable tunnel monitoring."
    )
    monitor_destination_ip = models.CharField(
        max_length=255, blank=True, null=True, # Use CharField for flexibility (IP/FQDN)
        help_text="Destination IP or FQDN to ping for monitoring."
    )
    natural_key_field_names = ["name"]
    monitor_profile = models.ForeignKey(
        TunnelMonitorProfile, on_delete=models.SET_NULL, related_name="ipsec_tunnels",
        blank=True, null=True, help_text="Tunnel Monitor Profile to use."
    )
    ...
    def clean(self):
        ...
        if self.enable_tunnel_monitor:
            if not self.monitor_destination_ip:
                raise ValidationError({"monitor_destination_ip": "Destination IP is required when tunnel monitoring is enabled."})
            if not self.monitor_profile:
                raise ValidationError({"monitor_profile": "Monitor Profile is required when tunnel monitoring is enabled."})
...
class IPSecProxyID(models.Model):
    ...
    class Meta: verbose_name = "IPSec Proxy ID"; verbose_name_plural = "IPSec Proxy IDs"; ordering = ["tunnel", "local_subnet", "remote_subnet"]
    def __str__(self): return f"{self.local_subnet or 'any'} <-> {self.remote_subnet or 'any'} ({self.protocol})"