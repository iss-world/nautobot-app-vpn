...
from nautobot.apps.forms import (
    NautobotModelForm, DynamicModelChoiceField, BootstrapMixin, NautobotFilterForm,
    DynamicModelMultipleChoiceField, APISelectMultiple # Keep for 'devices' field
)
...
from nautobot_app_vpn.models import (
    IPSECTunnel, IPSecProxyID, IKEGateway, IPSecCrypto, TunnelMonitorProfile, TunnelRoleChoices
)
...
class IPSECTunnelForm(NautobotModelForm):
    ...
    monitor_destination_ip = forms.CharField(required=False, label="Monitor Destination IP / FQDN", widget=forms.TextInput(attrs={"placeholder": "e.g., 8.8.8.8"}))
    ...
    class Meta:
        model = IPSECTunnel
        fields = [
            "name", "description", "devices", "ike_gateway", "ipsec_crypto_profile",
            "tunnel_interface", # "bind_interface", removed from here
            "role",
            "enable_tunnel_monitor", "monitor_destination_ip", "monitor_profile",
            "status",
        ]
        widgets = {
    ...
    def clean(self):
        ...
        if monitor_enabled:
            if not dest_ip: self.add_error("monitor_destination_ip", "This field is required when tunnel monitoring is enabled.")
            if not profile: self.add_error("monitor_profile", "This field is required when tunnel monitoring is enabled.")
...
class IPSecProxyIDForm(BootstrapMixin, forms.ModelForm):
     class Meta: model = IPSecProxyID; fields = [ "local_subnet", "remote_subnet", "protocol", "local_port", "remote_port",]
     def __init__(self, *args, **kwargs): super().__init__(*args, **kwargs); self.fields["local_subnet"].required = False; self.fields["remote_subnet"].required = False; self.fields["protocol"].required = False; self.fields["protocol"].initial = "any"
     def clean(self):
        if not hasattr(self, "cleaned_data"): return None
        cleaned_data = self.cleaned_data; local_subnet = cleaned_data.get("local_subnet"); remote_subnet = cleaned_data.get("remote_subnet")
        if cleaned_data.get("DELETE", False): return cleaned_data
        field_values = [v for k, v in cleaned_data.items() if k not in ('DELETE', 'id', 'tunnel')]
        if not any(field_values): return cleaned_data
        has_other_details = any(v for k, v in cleaned_data.items() if k in ('protocol', 'local_port', 'remote_port') and v)
        if not local_subnet and not remote_subnet and has_other_details: raise forms.ValidationError("At least one subnet (local or remote) must be provided if other Proxy ID details like protocol or ports are entered.")
        return cleaned_data


# 🔹 Inline formset for use in UI view (Keep as is)
IPSecProxyIDFormSet = inlineformset_factory(
    parent_model=IPSECTunnel, model=IPSecProxyID, form=IPSecProxyIDForm,
...
class IPSECTunnelFilterForm(NautobotFilterForm):
    ...
    ipsec_crypto_profile = DynamicModelChoiceField(queryset=IPSecCrypto.objects.all(), required=False, label="IPSec Crypto Profile")
    enable_tunnel_monitor = forms.NullBooleanField(required=False, widget=forms.Select(choices=[('', '---------'), ('true', 'Yes'), ('false', 'No')]))
    monitor_profile = DynamicModelChoiceField(queryset=TunnelMonitorProfile.objects.all(), required=False, label="Monitor Profile")
    ...
    fieldsets = (("Tunnel Filters", ("q", "devices", "ike_gateway", "role", "status")), ("Monitoring", ("enable_tunnel_monitor", "monitor_profile")),)