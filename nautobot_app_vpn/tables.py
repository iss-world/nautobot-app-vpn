...
from nautobot.apps.tables import (
    BaseTable, ToggleColumn, ButtonsColumn, StatusTableMixin,
    BooleanColumn, LinkedCountColumn # Ensure LinkedCountColumn is imported
)
...
class IKECryptoProfileTable(StatusTableMixin, BaseTable):
    ...
    class Meta(BaseTable.Meta):
        model = IKECrypto
        fields = ("pk", "name", "encryption", "authentication", "dh_group", "lifetime", "lifetime_unit", "status", "description", "actions")
...
class IPSecCryptoProfileTable(StatusTableMixin, BaseTable):
    ...
    class Meta(BaseTable.Meta):
        model = IPSecCrypto
        fields = ("pk", "name", "encryption", "authentication", "dh_group", "protocol", "lifetime", "lifetime_unit", "status", "description", "actions")
...
class IKEGatewayTable(StatusTableMixin, BaseTable):
    ...
    class Meta(BaseTable.Meta):
        model = IKEGateway
        fields = ("pk","name","local_devices_display","local_locations_display","bind_interface","local_ip_type","local_ip","peer_identifier","peer_location_display","peer_ip_type","peer_ip","ike_version","authentication_type","ike_crypto_profile","enable_nat_traversal","enable_dpd","status","description","actions","local_devices","peer_devices","local_locations","peer_locations","peer_device_manual","peer_location_manual","local_id_type","local_id_value","peer_id_type","peer_id_value","exchange_mode","enable_passive_mode","local_platform","peer_platform","dpd_interval","dpd_retry","liveness_check_interval",)
        default_columns = ("pk","name","local_devices_display","local_platform","local_ip","peer_identifier","peer_platform","peer_ip","ike_version","authentication_type","ike_crypto_profile","status","actions",)
        order_by = ("name",)
...
class IPSECTunnelTable(StatusTableMixin, BaseTable):
    ...
    class Meta(BaseTable.Meta):
        ...
        fields = (
            "pk","name", "role", "devices_display","ike_gateway", # Added role
            "ipsec_crypto_profile","tunnel_interface",
            "enable_tunnel_monitor","monitor_destination_ip", # Keep in available fields
            "monitor_profile","proxy_id_count","status",
            "description","actions","devices", # devices M2M field itself if needed
        )
        # Added 'role' to default columns
        default_columns = (
            "pk","name","role", "devices_display","ike_gateway","ipsec_crypto_profile", # Added role
            "tunnel_interface","enable_tunnel_monitor",
            "status","actions",
        )
...
class IPSecProxyIDTable(BaseTable): # No StatusMixin needed
    ...
    class Meta(BaseTable.Meta):
        model = IPSecProxyID
        fields = ("pk", "tunnel", "local_subnet", "remote_subnet", "protocol", "local_port", "remote_port", "actions",)
        default_columns = ("pk", "tunnel", "local_subnet", "remote_subnet", "protocol", "actions",)
...
class VPNDashboardTable(BaseTable): # No StatusMixin needed
     ...
     class Meta(BaseTable.Meta):
        model = VPNDashboard
        fields = ("pk", "name", "total_tunnels", "active_tunnels", "inactive_tunnels", "last_sync_status", "last_push_status", "actions")
        default_columns = ("pk", "name", "total_tunnels", "active_tunnels", "inactive_tunnels", "last_sync_status", "last_push_status", "actions")