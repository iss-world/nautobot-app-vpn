...
class IPSECTunnelUIViewSet(NautobotUIViewSet):
    ...
    queryset = IPSECTunnel.objects.select_related(
        ...
        "tunnel_interface", "monitor_profile"
    ).prefetch_related(
    ...
    def create(self, request, *args, **kwargs):
        ...
        return render(request, template_name, {
            "object": None, "object_type": object_type, "form": form,
            "formset": formset, "return_url": self.get_return_url(request),
            "editing": False,
        })
        ...
        if request.method == "POST":
            if form.is_valid() and formset.is_valid():
                try:
                 ...
                 if not form.is_valid(): logger.error(f"❌ Main Tunnel Form validation errors: {form.errors.as_json()}"); messages.error(request, "❌ Failed to update tunnel. Please check the main form.")
                 if not formset.is_valid(): logger.error(f"❌ ProxyID Formset validation errors: {formset.errors}"); messages.error(request, "❌ Failed to update proxy IDs. Please check the Proxy ID section.")

        return render(request, template_name, {
            "object": instance, "object_type": object_type, "form": form,
            "formset": formset, "return_url": self.get_return_url(request, instance),
            "editing": True,
        })
    ...
    def bulk_destroy(self, request):
         ...
         pks = request.POST.getlist("pk"); model = self.queryset.model
         if pks:
             try:
                 queryset = model.objects.filter(pk__in=pks); count = queryset.count()
                 if count > 0: logger.info(f"Deleting {count} {model._meta.verbose_name_plural}: {list(queryset.values_list('pk', flat=True))}"); queryset.delete(); messages.success(request, f"Deleted {count} {model._meta.verbose_name_plural}.")
                 else: messages.warning(request, "No matching tunnels found for deletion.")
             except Exception as e: logger.error(f"Error during bulk deletion of {model._meta.verbose_name_plural}: {e}"); messages.error(request, f"Error deleting tunnels: An unexpected error occurred.")
         else: messages.warning(request, "No tunnels selected for deletion.")
