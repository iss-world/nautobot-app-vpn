    ...
    fetch(url)
      ...
      .then(data => {
        ...
        if (data.meta) {
          ...
          const metaStatusOrder = Array.isArray(meta.status_order)
            ? meta.status_order.map(slug => String(slug).toLowerCase())
            : [];
          const statusOrder = [...new Set([...preferredStatusOrder, ...metaStatusOrder, ...Object.keys(statusCounts)])];
          const statusIcons = {
            active: "🟢",
            down: "🔻",
            failed: "🔴",
          ...
          const totalTunnels = meta.total_tunnels ?? meta.tunnels_count ?? 0;
          const roleCounts = meta.role_counts || {};
          const roleLabels = meta.role_labels || {};
          const preferredRoleOrder = ["primary", "secondary", "tertiary"];
          const metaRoleOrder = Array.isArray(meta.role_order)
            ? meta.role_order.map(slug => String(slug).toLowerCase())
            : [];
          const roleOrder = [...new Set([...preferredRoleOrder, ...metaRoleOrder, ...Object.keys(roleCounts)])];
          ...
          const nodes = meta.devices_count ?? meta.total_nodes_shown ?? data.nodes?.length ?? 0;
          const ha = meta.ha_pairs ?? meta.ha_pairs_shown ?? 0;
          const countries = meta.countries_count ?? new Set((data.nodes || []).map(n => n.data.country).filter(Boolean)).size;
          const platforms = meta.platforms_count ?? new Set((data.nodes || []).map(n => n.data.platform_name).filter(Boolean)).size;

          const segments = [
            ...statusSegments,
            `🔗 ${totalTunnels} Tunnels`,
            ...roleSegments,
            `🖥️ ${nodes} Devices (${countries} Countries, ${platforms} Platforms)`,
          ];
          if (ha) segments.push(`🟪 ${ha} HA pairs`);

          const syncTime = meta.last_synced
            ? `⏱ Last synced at: ${new Date(meta.last_synced).toLocaleString("en-GB", { timeZone: "UTC" })} UTC`
            : "⏱ Last synced at: —";
          segments.push(syncTime);

          if (meta.last_sync_status) {
            segments.push(`📋 Sync status: ${meta.last_sync_status}`);
          }

          if (statEl) statEl.textContent = segments.filter(Boolean).join(" | ");
        }
      })
      .catch(err => {
